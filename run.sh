...
# On Alpine 3.22, some libclang symlinks are broken, so using the real path here
python3 cparser.py --libclang /usr/lib/llvm19/lib/libclang.so.19.1.* --output /data/cxxindex.json --git /src/${1}/cxxdox.yml /src
